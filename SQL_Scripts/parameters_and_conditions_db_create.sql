...
CREATE TABLE IF NOT EXISTS trip_members (
    ...
    CONSTRAINT trip_members_unique UNIQUE (trip_id, first_name, last_name)
);
...
CREATE TABLE IF NOT EXISTS conditions (
    ...
    CONSTRAINT conditions_unique UNIQUE (current_date, time_of_day)
    /*
...
CREATE TABLE IF NOT EXISTS camera_parameters (
    ...
    image_distortion_mapping int ARRAY[] NOT NULL,
    date_added_to_db timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
--Send code/table to Aaryan
-- missing distortion (d) parameters
...
CREATE TABLE IF NOT EXISTS bag_files (
    ...
    bag_file_path varchar(50) NOT NULL,
...
CREATE TABLE IF NOT EXISTS post_trip_details (
    ...
    CONSTRAINT post_trip_id_unique UNIQUE (trip_id)
);

-- foreign keys
/*
FORMAT:
-- Reference: <table_to_change>_<table_to_connect> (table: <table_to_change>)
...
-- trip_members table
----------------------------------------------------------------------
-- Reference: trip_members_trips(table: trip_members)
...
-- Reference: trip_base_stations_trips (table: trip_base_stations)
ALTER TABLE trip_base_stations ADD
    FOREIGN KEY (trip_id)
    REFERENCES trips (id)  
    NOT DEFERRABLE 
...
----------------------------------------------------------------------
-- Reference: pre_trip_details_trips (table: pre_trip_details)
...
----------------------------------------------------------------------
-- Reference: post_trip_details_trips (table: post_trip_details)
